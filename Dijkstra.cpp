...
int main() {
    ...
    for(int v=end; v!=-1; v=parent[v]) {
        path.insert(path.begin(), v); // here we build path in correct order
    }
    for(int i=0;i<path.size();i++) {
        cout<<path[i];
        if(i!=path.size()-1) cout<<" -> "; // this is for print arrow between nodes
    }
    cout<<"\nTotal cost: "<<d[end]<<endl; // this is for print total distance
    return 0;
}
